...
class row_transformer:
    ...
    def check_and_find_start_row_index(self):
        logging.debug(
            'Checking to ensure input start row exists ' + str(self.start_row))
        if self.start_row in self.input_df.columns:
            logging.debug('Found column ' + str(self.start_row) +
                          ' in input data frame.')
            ...
            logging.error(
                "Could not The input_df argument is not a pandas dataframe.")
            exit - 1

    def transform_rows(self):
        logging.info(
            'Starting row transformation using starting row ' + str(self.start_row))
        numCols = len(self.input_df.columns)
        allHeadings = list(self.input_df.columns.values)
        dataFrameHeadings = allHeadings[:2]
        ...
        for index, row in self.input_df.iterrows():
            newRowsInitialCols = []
            logging.debug("Processing row " + str(index) +
                          "/" + str(self.input_df.shape[0]))
            ...
            for column in range(self.start_row_ind, numCols):
                ...
                newRowDf = pd.DataFrame(
                    [newRowData], columns=dataFrameHeadings)
